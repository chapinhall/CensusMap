...
def merge(csv_filename, geojson_filename, id_csv_col, id_geojson_col, output_name):
    df = read_csv(csv_filename, id_csv_col)
    geojson = read_json(geojson_filename)
    # Iniitialize unmatched records
    unmatched_json_records = []
    for feature in geojson['features']:
        unmatched_json_records.append(feature['properties'][id_geojson_col])
    matched = 0
    for row in df.itertuples(index=False):
        for feature in geojson['features']:
            #the first position of the df is the id column
            if str(row[0]) == str(feature['properties'][id_geojson_col]):
                matched += 1
    ...
    write_unmatched(unmatched_json_records, csv_filename)

    # save merged datasets to an output geojson file
    json_string = json.dumps(geojson)
    print('Saving file to {}'.format(output_name))
    with open(output_name, 'w') as f:
        f.write(json_string)
    f.close()

def write_unmatched(unmatched_json_records, csv_filename):
    csv_out = csv_filename.split("/")[-1].split(".")[0] + "_unmatched.csv"
...
merge("./tract-level-indicators-for-cnat.csv", "./tracts_clipped.geojson", "geoid", "geoid10", "./tracts_clipped_withAttr.geojson")

merge("./chapin-hall-estimates-of-program-eligible-youth-by-community-area.csv", "./Boundaries - Community Areas (current).geojson",
       "cca", "area_numbe", "./commAreas_withAttr.geojson")
